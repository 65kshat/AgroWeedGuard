...
if uploaded_files:
    ...
    if st.button("🚀 Run All Models", use_container_width=True):
        ...
        for image_number, uploaded_file in enumerate(uploaded_files, start=1):
            ...
            st.subheader(f"Image {image_number}: "
            ...
            try:
                ...
                # Faster R-CNN
                start = time.perf_counter()
                frcnn_result = run_fasterrcnn(image)
                frcnn_time = (time.perf_counter() - start)
                ...
                vit_time = (time.perf_counter() - start)
                ...
                frcnn_count = len(frcnn_detections)

                yolo_best = (max(yolo_detections, key=lambda x: x["confidence"]) if yolo_detections else None)
                frcnn_best = (max(frcnn_detections, key=lambda x: x["confidence"]) if frcnn_detections else None)

                comparison_data = [{"Model": "YOLOv8", 
                                    "Type": "Detection", 
                                    "Output": (yolo_best["class_name"] if yolo_best else "No Detection"),
                                    "Detections": yolo_count, 
                                    "Confidence": (f"{yolo_best['confidence'] * 100:.2f}%" if yolo_best else "-"),
                                    "Time (s)": round(yolo_time, 3)},

                                    {
                                    "Model": "Faster R-CNN",
                                    "Type": "Detection",
                                    "Output": (frcnn_best["class_name"] if frcnn_best else "No Detection"),
                                    "Detections": frcnn_count,
                                    "Confidence": (f"{frcnn_best['confidence'] * 100:.2f}%" if frcnn_best else "-"),
                                    ...
                                    "Detections": "-",
                                    "Confidence": (f"{cnn_result['confidence'] * 100:.2f}%"),
                                    ...
                                    "Detections": "-",
                                    "Confidence": (f"{vit_result['confidence'] * 100:.2f}%"),
                ...
                with col2:
                    ...
                    st.image(frcnn_image, use_container_width=True)

                    if frcnn_detections:
                        for detection in (frcnn_detections):
                            st.write(f"Detection:"
                                        f"{detection['class_name']}")
